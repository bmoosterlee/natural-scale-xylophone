...
public class NoteEnvironment implements Runnable{
    ...
    public void run() {
        ...
        int sampleLookAhead = (int) (frameTime*1000);

        while(true) {
            writeSample();

            long expectedSampleCount = (System.nanoTime()-timeZero) * SAMPLE_RATE;
            if(sampleCount > expectedSampleCount+sampleLookAhead){
                try {
                    Thread.sleep(0,sampleLookAhead/2);
